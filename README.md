...
ResumeMatch Pro は、**日本での就職を目指す海外エンジニア（外国人開発者）向け**の Web アプリケーションです。候補者が自分のレジュメ（履歴書）をアップロードすると、LLM を活用した **AI マッチングエンジン** が日本のエンジニア求人と自動マッチングし、応募や企業への問い合わせまでを一気通貫でサポートすることを目指しています。AI は各求人ごとに、総合スコアと「技術 / 学歴 / 経験 / プロジェクト / ソフトスキル」の 5 つの観点による **マッチ度スコア**、そのスコアサマリー、**強み・弱みの分析**、そして **面接に向けた具体的な対策** を自動生成します。 
 
このリポジトリは、Next.js（App Router）をベースに、TanStack Query、Zustand、nuqs、Zod、Supabase、AWS S3、Resend などのモダンなフロントエンド / フルスタック技術と **AI マッチングパイプライン** を組み合わせた、**ポートフォリオ集の一環**です。
